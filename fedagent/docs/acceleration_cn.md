...
> **数值等价**（full-loop max|Δ|=1.13e-5，bf16 噪声）。PPO critic reload（§7.1）与 per-client
> WebShop/ALFWorld service 路由（§7.3）也已落地。它是唯一不破坏可复现性的单节点
> 杠杆。windowed 默认值的 release blocker 也已修复（§7.5）。
...
### Lever #3 —— 一轮内并行 client *（多节点；最大吞吐；bit-equivalent）*
`for c in selected` → 并发，每个 client 在自己的 GPU 分配/节点上。**与串行数值完全一致**
（FedAvg 与顺序无关；每个 client 的 env seed 是 `base_seed + round*100 + client`
（`run_fed.py:635`），*按 client 索引，不依赖顺序*）。在 train 部分最多可达 `clients_per_round×`。
**需要 ≥2 节点 / 足够的 GPU 来隔离** —— 单节点上它只是争用（铁律）。注意：把
4 GPU 拆成 2+2 会改变 FSDP `world_size` → 分片布局（聚合器动态读取
`world_size_of`，能处理；相同 global batch ⇒ 相同数值）。
...
| 硬件 | 先做 | 大杠杆 | 跳过（争用） |
|---|---|---|---|
| **单 4-GPU 节点**（默认） | **#2**（免费、零风险） | **#4**（唯一触及那 88% 的） | #1, #3 |
...
| #3 parallel clients | 无（FedAvg 与顺序无关、seed 按 client 索引） | ✅ 安全 |
...
- **Phase 2 —— #4 原型。✅ 已完成（本次会话，§7）。** 仅 overlay 构建；在 smoke 上做了 A/B 验证：
  per-round −43%（§7.1）、**cross-round −62% 且整个 run 只有一次冷启动（§7.2）**、PPO critic
  reload（§7.1）与 per-client service routing（§7.3）均已落地；全程 full-loop max\|Δ\|=1.13e-5
  EQUIVALENT。**剩余：** 一次端到端的 WebShop/ALFWorld 持久化 run + 更大 step 的等价 A/B。
...
### 7.4 Eval/训练 GPU 共享（`eval_mode`：inline / parallel / shared / worker）—— 4 种已验证
...
`worker` 在 `i==0` 评该 round 的*起始*模型，其余 mode 在 merge 后评。
...
"eval ∥ train" 要么 **GPU 不相交**、要么 **缩小的第二引擎**、要么 **复用那一个热引擎** —— 故 4 种 mode：
...
**饱和 4-GPU 论文场景 → `worker`。** `n_gpus_per_node=4` 时没有空闲卡，`parallel` 不适用、`shared` 每轮付
一次 eval cold-start。`worker` 复用**热** rollout 引擎（无第二 vLLM → 无 OOM、无 cold-start），每轮 eval
便宜、保住 `cross_round` 速度。端到端已验证（2-round smoke 上 **703s** vs shared **874s**；论文 70-round /
15-eval 规模下差距拉大，`shared` 付 ~15 次 cold-start，`worker` 付 ~0）。
...
### 7.6 剩余（已排序）
1. **放大等价 A/B** —— 更多 step、真实 env、PPO（等价目前在 TinyGuess 2-step 上验证）。
2. 长时程下的 **vLLM sampler-RNG / /dev/shm teardown**（没咬到 smoke；观察并按需加
   `aggressive_empty_cache` / `SamplingParams.seed`）。
3. **论文级 worker-mode run** —— worker eval 已在 2-round smoke 验证；在 70-round 真实栈上确认热引擎
   eval 的长时程稳定性。
...
| [run_fed.py](../fed/run_fed.py) | **#4 per-round：** `persistent` 标志 + `run_round_persistent()` + run-loop 分支。**#4 cross-round：** `cross_round` 标志 + `BgProc` + `_wait_signal` + `stop_persistent_cross_round` + signal-file 握手。**routing：** `client_service_url()` + plan `service_url` + `FEDAGENT_SERVICE_URL_FILE`。**eval modes（§7.4）：** `eval_mode` inline/parallel/shared/worker —— `_build_eval`/`eval_global`/`launch_eval_async`/`collect_eval`；per-round eval **每 round**（`if do_eval:`，非 `r%test_freq`）；`cross_round`+inline → 自动回退 per-round。**#2：** `prewarm_next_round_services()` + `_wait_services_healthy()`。**windowed：** `history_length_env()` |
...
| [alfworld_env.py](../envs/alfworld/alfworld_env.py)、[webshop_env.py](../envs/webshop/webshop_env.py) | `history_length` 先读 `FEDAGENT_HISTORY_LENGTH`（优先），再读 spec；service URL 经由 `resolve_service_url`（per-client 文件路由） |
...
> 持久化（§7.1）、**跨轮持久化（§7.2）**、PPO critic reload（§7.1）以及 **per-client service
> 路由（§7.3）** —— 均**已完成 + GPU 验证**（cross-round：整个 run 一次冷启动，−62%，
> max\|Δ\|=1.13e-5 EQUIVALENT），仅 overlay，**未提交**。剩余：一次端到端的 multi-service
> WebShop/ALFWorld 持久化 run（路由已单元测试）+ 一次更大 step 的等价 A/B。原先"切换 7 份
> env spec + 重新生成 176 份配置"的方案**已被取代** —— auto-map + `FEDAGENT_HISTORY_LENGTH`
> 方法两者都不需要。
